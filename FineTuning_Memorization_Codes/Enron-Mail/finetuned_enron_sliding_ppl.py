...
def calculate_sliding_window_perplexity(model, tokenizer, text, window_size=50):
    """Compute perplexity over a sliding window approach."""
    tokens = tokenizer.tokenize(text)
    perplexities = []

    for i in range(0, len(tokens) - window_size + 1, window_size):  
        window_text = tokenizer.convert_tokens_to_string(tokens[i:i+window_size])
        inputs = tokenizer(window_text, return_tensors="pt", padding=True, truncation=True).to(device)
...
def generate_responses_and_perplexities(questions, model, tokenizer, device, max_length=1024, window_size=50):
    responses = []
    sliding_perplexities = []
    ...
    for question in tqdm(questions, desc="Generating responses and computing perplexities"):
        ...
        window_perplexities = calculate_sliding_window_perplexity(model, tokenizer, generated_text, window_size)
        sliding_perplexities.append(window_perplexities)
    
    return responses, sliding_perplexities
...
def main(args):
    ...
    responses, sliding_window_perplexities = generate_responses_and_perplexities(
        questions, model, tokenizer, device, window_size=50
    )
    ...
    perplexity_data = {
        "questions": questions,
        "responses": responses,
        "sliding_window_perplexities": sliding_window_perplexities
    }
