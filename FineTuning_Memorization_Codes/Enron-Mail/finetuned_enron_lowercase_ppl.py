...
def calculate_lowercase_perplexity(model, tokenizer, text):
    """Compute perplexity for the text after converting it to lowercase."""
...
def generate_responses_and_perplexities(questions, model, tokenizer, device, max_length=1024):
    ...
    for question in tqdm(questions, desc="Generating responses and computing perplexities"):
        ...
        lowercase_perplexity = calculate_lowercase_perplexity(model, tokenizer, generated_text)
        lowercase_perplexities.append(lowercase_perplexity)
    
    return responses, lowercase_perplexities
...
def main(args):
    ...
    responses, lowercase_perplexities = generate_responses_and_perplexities(questions, model, tokenizer, device)
    ...
    perplexity_data = {
        "questions": questions,
        "responses": responses,
        "lowercase_perplexities": lowercase_perplexities
    }
