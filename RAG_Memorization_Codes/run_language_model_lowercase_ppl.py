...
def compute_lowercase_perplexity(generator, prompt, max_gen_len, temperature, top_p, flag_llm):
    """
    Compute perplexity using lowercase transformation.
    ...
    Returns:
        tuple: (generated text, lowercase perplexity value)
    """
    prompt = prompt.lower()
    if flag_llm == "gpt":
        results = generator.invoke(prompt)
        generated_text = results.lower()
    ...
    generated_text = results[0]['generation'].lower()
...
def main(
    ...
    """
    Main function to generate text and compute lowercase perplexity.
    ...
    for prompt in all_prompts:
        ans, perplexity = compute_lowercase_perplexity(generator if flag_llm == "llama" else llm, prompt, max_gen_len, temperature, top_p, flag_llm)
        answers.append(ans)
    ...
    # Save perplexities
    with open(f"./Inputs&Outputs/{path}/lowercase_perplexities-{ckpt_dir}.json", 'w', encoding='utf-8') as file:
        file.write(json.dumps(perplexities))

    print("Text generation and lowercase perplexity computation complete!")
