...
def compute_perplexity(generator, prompt, max_gen_len, temperature, top_p, flag_llm):
    """
    Compute perplexity manually if log_probs is unavailable.
    ...
    Returns:
        tuple: (generated text, perplexity value)
    """
    if flag_llm == "gpt":
        results = generator.invoke(prompt)
        generated_text = results
        perplexity = None  # OpenAI models don't return logits
    ...
    # Tokenize prompt + generated text
    ...
    log_probs = F.log_softmax(logits[:, :-1, :], dim=-1)  # Shape: (1, seq_len-1, vocab_size)
...
def main(
    ...
    """
    Main function to generate text and compute perplexity.
    ...
    for prompt in all_prompts:
        ans, perplexity = compute_perplexity(generator if flag_llm == "llama" else llm, prompt, max_gen_len, temperature, top_p, flag_llm)
        answers.append(ans)
    ...
    # Save perplexities
    with open(f"./Inputs&Outputs/{path}/perplexities-{ckpt_dir}.json", 'w', encoding='utf-8') as file:
        file.write(json.dumps(perplexities))

    print("Text generation and perplexity computation complete!")
